...
import type { SingleInstructionPlan, SequentialInstructionPlan } from '@solana/instruction-plans';
...
test('plugin createMint defaults mintAuthority to payer address', async t => {
    ...
    // initializeMint2 — mintAuthority at index 1 (after mint at index 0)
    // Actually let's check the instruction data instead... or just verify the account structure.
    // initializeMint2 only has mint as an account, mintAuthority is in the data args.
    // So let's verify the createAccount instruction has the right payer.
    t.is(accounts[0][0], payer.address); // createAccount payer
});
